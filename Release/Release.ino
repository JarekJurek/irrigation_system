...
void startOnCondition(int valveNr, int runTime, int &count, int &lastMin, int virtualPin, bool &valveButtonOn, int startH, int startM) {
...
void loop() {
  ...
  rainDetected = !digitalRead(rainSensor);  // sensor's 1 - no rain, 0 - rain
  ...
  // main functions
  runOnCondition(valve1, runTime1, count1, lastMin1, virtualPin1, valveButton1, startH1, startM1);
  stopOnCondition(valve1, runTime1, count1, lastMin1, virtualPin1, valveButton1);

  runOnCondition(valve2, runTime2, count2, lastMin2, virtualPin2, valveButton2, startH2, startM2);
  stopOnCondition(valve2, runTime2, count2, lastMin2, virtualPin2, valveButton2);

  runOnCondition(valve3, runTime3, count3, lastMin3, virtualPin3, valveButton3, startH3, startM3);
  stopOnCondition(valve3, runTime3, count3, lastMin3, virtualPin3, valveButton3);

  runOnCondition(valve4, runTime4, count4, lastMin4, virtualPin4, valveButton4, startH4, startM4);
  stopOnCondition(valve4, runTime4, count4, lastMin4, virtualPin4, valveButton4);

  runOnCondition(valve5, runTime5, count5, lastMin5, virtualPin5, valveButton5, startH5, startM5);
  stopOnCondition(valve5, runTime5, count5, lastMin5, virtualPin5, valveButton5);

  runOnCondition(valve6, runTime6, count6, lastMin6, virtualPin6, valveButton6, startH6, startM6);
